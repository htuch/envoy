...
envoy::api::v3alpha::Cluster AdsIntegrationTest::buildCluster(const std::string& name) {
...
envoy::api::v3alpha::Cluster AdsIntegrationTest::buildRedisCluster(const std::string& name) {
  return TestUtility::parseYaml<envoy::api::v3alpha::Cluster>(fmt::format(R"EOF(
      name: {}
      connect_timeout: 5s
      type: EDS
...
envoy::api::v3alpha::Listener AdsIntegrationTest::buildListener(const std::string& name,
                                                                const std::string& route_config,
                                                                const std::string& stat_prefix) {
  return TestUtility::parseYaml<envoy::api::v3alpha::Listener>(fmt::format(
      R"EOF(
      name: {}
      address:
        socket_address:
...
envoy::api::v3alpha::Listener AdsIntegrationTest::buildRedisListener(const std::string& name,
...
AdsIntegrationTest::buildRouteConfig(const std::string& name, const std::string& cluster) {
  return TestUtility::parseYaml<envoy::api::v3alpha::RouteConfiguration>(fmt::format(R"EOF(
      name: {}
      virtual_hosts:
      - name: integration
        domains: ["*"]
...
envoy::admin::v3alpha::ClustersConfigDump AdsIntegrationTest::getClustersConfigDump() {
  auto message_ptr =
      test_server_->server().admin().getConfigTracker().getCallbacksMap().at("clusters")();
  return dynamic_cast<const envoy::admin::v3alpha::ClustersConfigDump&>(*message_ptr);
}

envoy::admin::v3alpha::ListenersConfigDump AdsIntegrationTest::getListenersConfigDump() {
  auto message_ptr =
      test_server_->server().admin().getConfigTracker().getCallbacksMap().at("listeners")();
  return dynamic_cast<const envoy::admin::v3alpha::ListenersConfigDump&>(*message_ptr);
}
...
envoy::admin::v3alpha::RoutesConfigDump AdsIntegrationTest::getRoutesConfigDump() {
  ...
  return dynamic_cast<const envoy::admin::v3alpha::RoutesConfigDump&>(*message_ptr);
}
